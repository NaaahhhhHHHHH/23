...
const EmployeeTable = () => {
  ...
  const handleAddOrUpdate = async (values) => {
    try {
      let res = currentEmployee ? await updateData('employee', currentEmployee.id, values) : await createData('employee', values);
      loadEmployees();
    ...
    <>
      ...
      >
        <Form form={form} onFinish={handleAddOrUpdate} labelCol={{ span: 6 }} wrapperCol={{ span: 15 }}>
          ...
          <Form.Item name="password" label={currentEmployee ? 'New Password' : 'Password'} rules={[{ required: currentEmployee ? false : true }]}>
            <Input.Password />
          </Form.Item>
